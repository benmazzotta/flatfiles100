...
f_appendbalances <- function(blockid, balances_blockend) {
  ...
  ## Read the last 99 chunks of balances into rolling balances
  ##      Why not all time? Eventually the file size will blow up.
  balances_rolling100 <- read_csv(paste0("balances_t_", blockid-1, ".csv"), col_types="cTii") %>% 
    filter(chunk >= max(chunk)-98)
  ## Append balances_blockend
  print("End of period balances for current period.")
  balances_rolling100 <- bind_rows(balances_rolling100, balances_blockend)
  # glimpse(balances_rolling100)
  ## Write to disk
  write_csv(balances_rolling100, path=paste0("../data/balances_t_", blockid,".csv"))
  print("Balances saved to disk.")
}
...
trx <- read_csv("trx_block_1.csv", col_types="ccciT")
...
for(blockcounter in 2:n_blocks) {
  ...
  print("Daily cash averages complete.")
}
